/**
 * ═══════════════════════════════════════════════════════
 *  praxiscare STATS — Couche de tracking pédagogique
 *  v1.0 · 2026 · fabsav68-lgtm.github.io/praxiscare
 * ═══════════════════════════════════════════════════════
 *
 *  INTÉGRATION dans un module HTML (3 lignes) :
 *  ─────────────────────────────────────────────
 *  <script src="praxiscare-stats.js"></script>
 *  <script>
 *    PraxisStats.init('simgluco');   // id du module
 *  </script>
 *
 *  USAGE dans le code du module :
 *  ─────────────────────────────
 *  PraxisStats.track('onglet', 'Prescription')
 *  PraxisStats.track('quiz', { q: 2, correct: true, ms: 8200 })
 *  PraxisStats.track('ia_question', { module: 'simgluco', chars: 142 })
 *  PraxisStats.track('scenario_etape', { etape: 3, label: 'Prescription' })
 *  PraxisStats.setStudent('Marie D.')   // si l'étudiant saisit son prénom
 * ═══════════════════════════════════════════════════════
 */
...
const PraxisStats = (() => {
  ...
  // ── CONFIG ──────────────────────────────────────────
  const VERSION    = '1.0';
  const LS_KEY     = 'praxiscare_stats_v1';
  const LS_STUDENT = 'praxiscare_student_id';
...
const SUPABASE_ANON = 'sb_publishable_TNksbociGaWCY53M4wCAXg_faOxEqKt';
  ...
  function saveStore(data) {
    try {
      ...
      console.warn('[PraxisStats] localStorage full');
    }
  }
  ...
  function init(moduleId) {
    ...
    console.log(`[PraxisStats] Init · module=${moduleId} · session=${_sessionId}`);
  }
  ...
  function exportCSV() {
    ...
    a.href     = url;
    a.download = `praxiscare-stats-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  function reset(confirm = false) {
    if (!confirm) {
      console.warn('[PraxisStats] Passez reset(true) pour confirmer la suppression');
      return false;
    }
    localStorage.removeItem(LS_KEY);
    localStorage.removeItem(LS_STUDENT);
    console.log('[PraxisStats] Données supprimées');
    return true;
  }
...
// Rendre global
window.PraxisStats = PraxisStats;
